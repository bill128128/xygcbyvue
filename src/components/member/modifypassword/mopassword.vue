<template>
	<div id="mopassword">
		<form class="form">
                ...
                <div class="tc">
                    <button type="button" class="submit" @click="doit">提交</button>
                </div>
            </form> 
	</div>
</template>
...
<script>
	export default{
		...
		methods:{
			...
			doit(e){
				if(this.checkEmpty(this.pwdInfo.oldpwd,'原密码') == false) return false;
                if(this.checkEmpty(this.pwdInfo.newpwd,'新密码') == false) return false;
                if(this.checkEmpty(this.pwdInfo.repwd,'确认新密码') == false) return false;
                if(this.pwdInfo.newpwd != this.pwdInfo.repwd){
                	return false
                }
				e.preventDefault();
				this.pwdInfo.memberid = parseInt(window.localStorage.getItem('memberid'));
