        	...
        	getContent(){            //获取内容
                ...
                this.$store.dispatch('buymembership/getMemberList',this.memberid).then(() => {
					if(this.$store.state.buymembership.code==200){
                        this.content = this.$store.state.buymembership.carmember[2];
                        this.directorsId = this.$store.state.buymembership.carmember[2].id;
...
.fade-enter-active, .fade-leave-active {
   ...
   .pwd-wrap li i{
	...
	.goldmember{
		...
		width: 8.27rem;
		height: 14.42rem;
		background: url(../../../static/img/jzhybg.png) no-repeat;
