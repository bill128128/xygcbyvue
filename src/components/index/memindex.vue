...
<script>
	...
	export default{
		...
		computed:{
            ...
            region(){
            	return this.$store.state.loginIndexData.region;
            	
            },
            car(){
            	return this.$store.state.loginIndexData.car;
            },
        },
        created(){
        	this.getbanner();
        	...
        	this.info.token = window.localStorage.getItem('date');
        	//页面加载完毕调用接口函数
        	this.$store.dispatch('loginIndexData/getLoginIndexData',this.info).then(()=>{       		
        		if(this.$store.state.loginIndexData.code==200){
        			window.localStorage.setItem('cityid',this.region.id);
        			var lists = [];
        			for(var i=0; i<this.$store.state.loginIndexData.roll.length; i++){
        				lists.push(this.$store.state.loginIndexData.roll[i].title)
        			}
        			//console.log(this.list);
        			window.localStorage.setItem('roll',lists);
            		
				}else if(this.$store.state.loginIndexData.code==40000){
					this.$toast({
						message:'您还没有登录！'
					})
					...
					this.$toast({
						message:this.$store.state.loginIndexData.msg,
					});
				}
			}).catch((e) => {              
		...
		methods:{
			...
			getmemberid(){
				...
				memberid = window.atob(token).split(',')[0].split(':')[1];
